...
async function handleX3DHInitMessage(msg) {
  try {
    ...
    if (envelope.pqCiphertext && envelope.kemPublicKey && Crypto.receiveHybridSession) {
      console.log('[X3DH v6] Processing hybrid PQ+ECDH session init from', String(msg.from).replace(/[\r\n]/g, " "));
      const result = await Crypto.receiveHybridSession(msg.from, envelope);
      
      if (result.sessionReady || result.sessionEstablished) {
        console.log('[X3DH v6] Hybrid session established with', String(msg.from).replace(/[\r\n]/g, " "));
    ...
    if (envelope.ephemeralPublicKey) {
      console.log('[X3DH v5] Processing classical X3DH session init from', String(msg.from).replace(/[\r\n]/g, " "));
      const result = await Crypto.receiveSession(msg.from, envelope);
      
      if (result.sessionReady || result.sessionEstablished) {
        console.log('[X3DH v5] Classical session established with', String(msg.from).replace(/[\r\n]/g, " "));
    ...
    console.error('[X3DH] Failed to process init message:', String(err).replace(/[\r\n]/g, " "));
    return false;
  }
}
