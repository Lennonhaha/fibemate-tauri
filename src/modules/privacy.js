  ...
  try {
    ...
    if (envelope.pqCiphertext && envelope.kemPublicKey && Crypto.receiveHybridSession) {
      ...
      if (result.sessionReady || result.sessionEstablished) {
        console.log('[X3DH v6] Hybrid session established with', encodeURIComponent(JSON.stringify(msg.from)));
    ...
    if (envelope.ephemeralPublicKey) {
      console.log('[X3DH v5] Processing classical X3DH session init from', encodeURIComponent(JSON.stringify(msg.from)));
      const result = await Crypto.receiveSession(msg.from, envelope);
      
      if (result.sessionReady || result.sessionEstablished) {
        console.log('[X3DH v5] Classical session established with', encodeURIComponent(JSON.stringify(msg.from)));
    ...
    console.error('[X3DH] Failed to process init message:', encodeURIComponent(JSON.stringify(err)));
    return false;
  }
}
